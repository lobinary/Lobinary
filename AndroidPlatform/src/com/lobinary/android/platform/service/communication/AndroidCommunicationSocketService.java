...
import com.lobinary.android.platform.ui.fragment.MainContentContactFragment;
...
public class AndroidCommunicationSocketService extends CommunicationSocketService {
	...
	public boolean refreshConnectableList() {
		new Thread() {
			@Override
			public void run() {
				if (System.currentTimeMillis() - connectionMapVersionId < 10000) {
					logger.info("刷新时间间隔过短,请您稍后再试......");
				} else {
					logger.info("准备刷新可连接设备列表......");
					removeAllUnConnectionBean();
					MainContentContactFragment.contactHandler.sendMessage(new android.os.Message());
					connectionMapVersionId = System.currentTimeMillis();
					long connectionMapVersionIdTemp = connectionMapVersionId;// 连接刷新临时ID
