...
public class CreateChartServiceImpl {  
    ...
    private static final String CHART_PATH = "/apps/house/img/";  
    ...
    public static String 创建链家价格走势图(List<房屋统计信息> list){
    	double[] 上涨 = new double[list.size()];
    	double[] 下降 = new double[list.size()];
    	String[] 批次号 = new String[list.size()];
    	for (int i = 0; i < list.size(); i++) {
    		上涨[i] = (Double.parseDouble(list.get(i).get房屋涨价数量()));
    		下降[i] = (Double.parseDouble(list.get(i).get房屋降价数量()));
    		批次号[i] = list.get(i).get批次号();
		}
    	 double[][] data = new double[][]{  上涨, 下降 };  
         String[] rowKeys = {"up", "down"};  
         String[] columnKeys = 批次号;  
         CategoryDataset dataset = getBarData(data, rowKeys, columnKeys);  
         return createTimeXYChar("house price trend","batch no", "total house",  dataset, "price-trend"+DateUtil.getDate(new Date())+".png");
    }
    ...
    public void makeLineAndShapeChart() {  
        double[][] data = new double[][]{  
        	{0,3,1,126,13,148,21,17,359,0,329,168,43,1,14,19,0,5,2,6,2,3,2,1},  
        	{0,1,12,42,13,53,10,10,101,0,81,45,40,24,149,315,2,100,67,43,45,9,15,19}
        };  
        String[] rowKeys = {"涨价", "降价"};  
        String[] columnKeys = {"20170204000000","20170205000001","20170205000002","20170209090000","20170209140000","20170213110000","20170213160000","20170214000000","20170303090000","20170303150000","20170313000000","20170405000000","20170411000000","20170412000000","20170417000000","20170502000000","20170503000000","20170504000000 ","20170505000000","20170506000000","20170507000000","20170508000000","20170508172702","20170509120002"};  
        String[] columnKeysNew = new String[columnKeys.length];
        CategoryDataset dataset = getBarData(data, rowKeys, columnKeysNew);  
        createTimeXYChar("房屋价格变动走势图", "变动房源数量", "变动日期", dataset, "lineAndShap.png");  
    }  
    ...
    public static String createTimeXYChar(String chartTitle, String x, String y,  
        ...
        try {  
            ...
            // 将报表保存为png文件  
            ChartUtilities.writeChartAsPNG(fos_jpg, chart, 1200, 510);  
    ...
    public String createStackedBarChart(CategoryDataset dataset, String xName,  
        ...
        chart.setTitle(new TextTitle(chartTitle, new Font("宋体", Font.BOLD,  
                25)));  
        // 2 ．2.1:设置中文  
        // x,y轴坐标字体  
        Font labelFont = new Font("宋体", Font.BOLD, 12);  
