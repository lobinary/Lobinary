...
public interface 房屋信息数据库 {
	...
	@Insert("INSERT INTO 房屋基本信息 "
			...
			+ "抵押信息, 交易权属, 房屋用途, 产权所属, 房本备注, 联系人, 联系信息, 建房时间, 发布时间, 备注, 当前状态)"
			+ " VALUES"
			+ " (#{id}, #{房屋信息来源}, #{房屋唯一标识}, #{房屋类型}, #{名称}, #{标题}, #{子标题}, #{网址}, #{所在城市}, "
			+ "#{所在区县}, #{所在地点}, #{所在环数}, #{所在小区}, #{所在小区编号}, #{所在楼号}, #{坐标}, #{户型},"
			+ " #{建筑面积}, #{实用面积}, #{朝向}, #{装修}, #{供暖}, #{产权年限}, #{是否唯一}, #{所在楼层}, #{总楼层}, #{户型结构}, #{建筑类型},"
	...
	@Update("update 房屋基本信息 set 房屋信息来源 = #{房屋信息来源} ,房屋唯一标识 = #{房屋唯一标识},"
			...
			+ "联系人 = #{联系人},联系信息 = #{联系信息},建房时间 = #{建房时间},发布时间 = #{发布时间},备注 = #{备注},当前状态 = #{当前状态}"
            + " where id=#{id}")
	...
	@Select("SELECT * FROM house.房屋基本信息 where 房屋信息来源 = #{房屋信息来源} and 当前状态 = #{当前状态} limit #{限制条数}")
	...
	@Insert("INSERT INTO 小区基本信息 "
			+ "(id, 小区信息来源, 小区编号, 小区名称, 小区坐标, "
			+ "小区总楼数, 小区楼盘均价, 建筑日期, 建筑类型, 挂牌房源数量, 出租房源数量)"
			+ " VALUES"
			+ " (#{id}, #{小区信息来源}, #{小区编号}, #{小区名称}, #{小区坐标},"
			+ " #{小区总楼数}, #{小区楼盘均价}, #{ 建筑日期}, #{挂牌房源数量}, #{出租房源数量})")
