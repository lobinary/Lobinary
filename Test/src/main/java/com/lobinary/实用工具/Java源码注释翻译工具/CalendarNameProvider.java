 ...
 * 
 * #getDisplayName（String，int，int，int，Locale）getDisplayName}和{@link #getDisplayNames（String，int，int，Loc
 * ale）getDisplayNames}方法提供日历字段值名称。
 * 日历}。以下是日历常用字段及其为每个日历系统支持的值。
 * <TR>
 * <th>字段</th> <th>值</th> <th>描述</th>
 * </tr>
 * <td valign ="top"> {@ link Calendar＃MONTH} </td> <td valign ="top"> {@ link Calendar＃JANUARY} to {@l
 * ink Calendar＃UNDECIMBER} </td> <td> Month编号是基于0（例如，0-1月，...，11-12月）。
 * <td valign ="top"> {@ link Calendar＃DAY_OF_WEEK} </td> <td valign ="top"> {@ link Calendar＃SUNDAY} t
 * o {@link Calendar＃SATURDAY} </td> <td> Day （即，1  - 星期日，...，7  - 星期六）开始，星期编号为1。
 * <td valign ="top"> {@ link Calendar＃AM_PM} </td> <td valign ="top"> {@ link Calendar＃AM} to {@link C
 * alendar＃PM} </td> <td> 0 -  AM，1  -  PM </td>。
 * <th>日历类型</th> <th>字段</th> <th>值</th> <th>描述</th>
 * <td rowspan ="2"valign ="top"> {@ code"gregory"} </td> <td rowspan ="2"valign ="top"> {@ link Calend
 * ar＃ERA} </td> <td > 0 </td> <td> {@ link java.util.GregorianCalendar＃BC}（BCE）</td>。
 * <td> 1 </td> <td> {@ link java.util.GregorianCalendar＃AD}（CE）</td>
 * <td rowspan ="2"valign ="top"> {@ code"buddhist"} </td> <td rowspan ="2"valign ="top"> {@ link Calen
 * dar＃ERA} </td> <td > 0 </td> <td> BC（BCE）</td>。
 * <td> 1 </td> <td> B.E。 （佛教时代）</td>
 * <td rowspan ="6"valign ="top"> {@ code"japanese"} </td> <td rowspan ="5"valign ="top"> {@ link Calen
 * dar＃ERA} </td> <td > 0 </td> <td> Seireki（Meiji之前）</td>。
 * <td> 1 </td> <td> Meiji </td>
 * <TD> 2 </TD> <TD>大正</TD>
 * <td> 3 </td> <td> Showa </td>
 * <TD> 4 </TD> <TD>平成</TD>
 ...
 * <td rowspan ="2"valign ="top"> {@ code"roc"} </td> <td rowspan ="2"valign ="top"> {@ link Calendar＃E
 * RA} </td> <td > 0 </td> <td> ROC之前</td>。
 * <TD> 1 </TD> <TD>中华民国</TD>
 * <td rowspan ="2"valign ="top"> {@ code"islamic"} </td> <td rowspan ="2"valign ="top"> {@ link Calend
 * ar＃ERA} </td> <td > 0 </td> <td> Before AH </td>之前。
 * <TD> 1 </TD> <TD>安诺伊历（AH）</TD>
 * 由{@link java.text.spi.DateFormatSymbolsProvider}提供的日期时间符号。
 * 
...
public abstract class CalendarNameProvider extends LocaleServiceProvider {
     ...
     * 
     * 日历＃MONTH}。此方法不支持时区字段{@link Calendar＃ZONE_OFFSET}和{@link日历＃DST_OFFSET}。
     * 例如，如果{@code field}为{@link Calendar＃DAY_OF_WEEK}，则有效值为{@link Calendar＃SUNDAY}到{@link Calendar＃SATURDA
     * Y}（含）。
     * {@link日历＃SHORT_FORMAT}（{@link日历＃SHORT SHORT}），{@link日历＃SHORT_STANDALONE}，{@link日历＃LONG_FORMAT}（{@lin
     * k日历#LONG LONG}），{@link日历＃ LONG_STANDALONE}，{@link日历＃NARROW_FORMAT}或{@link日历＃NARROW_STANDALONE}。
     * <前>
     * getDisplayName（"gregory"，Calendar.DAY_OF_WEEK，Calendar.SUNDAY，Calendar.LONG_STANDALONE，Locale.ENGLIS
     * H）;。
     * </pre>
     * 
     * @param calendarType
     ...
     * 日历＃MONTH}。此方法不支持时区字段{@link Calendar＃ZONE_OFFSET}和{@link日历＃DST_OFFSET}。
     * {@link日历＃SHORT_FORMAT}（{@link日历＃SHORT SHORT}），{@link日历＃SHORT_STANDALONE}，{@link日历＃LONG_FORMAT}（{@lin
     * k日历＃长} LONG}），{@link日历＃LONG_STANDALONE}，{@link日历＃NARROW_FORMAT}或{@link日历＃NARROW_STANDALONE}。
     * {@link"＃JANUARY}，{@code"JANUARY"}，{@code"February"}到{@link日历＃FEBRUARY} {@link＃JANUARY}，{@code"代码"Feb
     * "}到{@link Calendar＃FEBRUARY}，等等。
     * <pre>
     * </PRE>
     * 
     * @param calendarType
